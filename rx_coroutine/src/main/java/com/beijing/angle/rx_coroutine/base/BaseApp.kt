...
open class BaseApp : Application() {
     ...
     * 缓存模式
     *
     * @param ONLY_NETWORK  该模式下，仅请求网络，不处理缓存；这是RxHttp默认的缓存模式
     *
     *@param ONLY_CACHE 该模式下，仅读取缓存，读取成功，直接返回；否则直接抛出异常
     *
     *@param NETWORK_SUCCESS_WRITE_CACHE  该模式下，直接请求网络，若请求成功，则写入缓存并返回；否则直接返回
     *
     *@param READ_CACHE_FAILED_REQUEST_NETWORK 该模式下，先读取缓存，读取成功，直接返回；否则将请求网络(请求成功，写入缓存)
     *
     *@param REQUEST_NETWORK_FAILED_READ_CACHE  该模式下，先请求网络，请求成功，写入缓存并返回；否则读取缓存
     *
     */
